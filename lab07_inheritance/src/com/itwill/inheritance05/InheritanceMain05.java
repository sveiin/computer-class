 ...
 *   o. 해시코드가 만족해야 하는 조건:
 *      1. 같은 객체에서 hashCode()를 여러번 호출하더라도 항상 같은 정수가 리턴되어야 함
 *      2. equals() 메서드의 결과가 true가 되는 두 객체의 해시코드 값은 같아야 함
 *      3. 해시코드 값이 같은 두 객체에서 equals() 메서드의 결과가 항상 true가 될 필요는 없음
 */

// equals() 메서드를 override 하면 hashCode() 메서드도 같이 override 해줘야 함
...
public class InheritanceMain05 {
    ...
    public static void main(String[] args) {
        ...
        // argument가 null이 아닌 경우에는 x.toString()이 리턴하는 문자열을 콘솔에 출력
        // argument가 null인 경우에는 콘솔에 "null"이라고 출력
        ...
        System.out.println(rand); //-> Random 클래스는 toString() 메서드를 override하지 않음
